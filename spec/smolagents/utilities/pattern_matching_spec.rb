RSpec.describe Smolagents::PatternMatching do
  describe ".extract_code" do
    it "extracts code from ruby code blocks" do
      ...
      result = described_class.extract_code(text)

      expect(result.code).to eq("define_tool(:search) do\n  puts 'hello'\nend")
    end
    ...
    it "extracts code from generic code blocks" do
      ...
      expect(result.code).to eq("def hello\n  puts 'world'\nend")
    end
    ...
    it "extracts code from HTML-style code tags" do
      ...
      expect(result.code).to eq("x = calculate(expression: \"1 + 2\")")
    end
    ...
    it "handles multiline HTML code tags" do
      ...
      expect(result.code).to eq("def greet\n  puts 'hi'\nend")
    end
    ...
    it "prefers ruby code blocks over generic ones" do
      ...
      expect(result.code).to eq("result = calculate(expression: \"1+2\")")
    end
    ...
    context "with small model formatting variations" do
      it "handles missing newline after triple backticks" do
        ...
        expect(result.code).to eq("result = calculate(expression: \"5 * 3\")")
      end
      ...
      it "handles uppercase Code tags" do
        ...
        expect(result.code).to eq("puts calculate(expression: \"10 / 2\")")
      end
      ...
      it "handles code blocks without language tag" do
        ...
        expect(result.code).to eq("final_answer(answer: 100)")
      end
      ...
      it "handles xml-style ruby tags" do
        ...
        expect(result.code).to eq("x = calculate(expression: \"2**8\")")
      end
    end
    ...
    context "when validating code" do
      ...
      it "accepts code with method calls" do
        ...
        expect(result.code).to include("calculate")
      end
      ...
      it "accepts code with final_answer" do
        ...
        expect(result.code).to eq("final_answer(answer: \"Paris\")")
      end
    ...
    context "with OpenAI Harmony format (gpt-oss models)" do
      it "extracts tool calls from harmony format" do
        ...
        expect(result.code).to eq('result = search(query: "hello world")')
      end
      ...
      it "handles gpt-oss-20b actual output format" do
        ...
        expect(result.code).to eq('result = searxng_search(query: "trending programming languages 2026")')
      end
      ...
      it "handles complex harmony output with multiple markers" do
        ...
        expect(result.code).to eq('result = searxng_search(query: "Ruby 4.0 features")')
      end
      ...
      it "handles tool calls with multiple arguments" do
        ...
        expect(result.code).to include("weather(")
        expect(result.code).to include('city: "Tokyo"')
        expect(result.code).to include('units: "celsius"')
      end
      ...
      it "converts harmony final answer to code" do
        ...
        expect(result.code).to eq('final_answer(answer: "The answer is 42")')
      end
    end
    ...
    context "with model-specific special tokens" do
      ...
      it "strips Llama BOS/EOS tokens" do
        text = "<s>```ruby\nresult = search(query: 'test')\n```</s>"
        result = described_class.extract_code(text)
      ...
      it "strips Mistral instruction markers" do
        text = "[INST]Find info[/INST]```ruby\nresult = search(query: 'ruby')\n```"
        result = described_class.extract_code(text)

        expect(result.code).to eq("result = search(query: 'ruby')")
      end

      it "strips tokenizer special tokens" do
        text = "<pad><pad>```ruby\nfinal_answer(answer: 42)\n```<unk>"
        result = described_class.extract_code(text)
    ...
    context "with thinking tags" do
      ...
      it "avoids code duplication when code appears in thinking and after" do
        ...
        expect(result.code).not_to include("x = 1\nx = 1")
      end
    end
    ...
    context "with [TOOL_CALLS] suffix" do
      it "strips TOOL_CALLS suffix and extracts code from ruby block" do
        ...
        expect(result.code).to eq(%(data = duckduckgo_search(query: "trending languages")\nfinal_answer(answer: data)))
        expect(result.code).not_to include("[TOOL_CALLS]")
      end
    end
    ...
    context "with tool_request markdown format" do
      it "extracts tool_request and converts to Ruby" do
        ...
        expect(result.code).to eq("result = get_data(id: 123)")
      end
    end
  end
  ...
  describe "malformed model outputs" do
    context "with text after final_answer" do
      it "extracts final_answer with trailing explanation" do
        ...
        expect(result.code).to eq("final_answer(answer: 50)")
      end
      ...
      it "extracts variable answer with trailing text" do
        ...
        expect(result.code).to eq("final_answer(answer: result)")
      end
      ...
      it "handles nested parens in answer with trailing text" do
        ...
        expect(result.code).to eq("final_answer(answer: calculate(1+2))")
      end
    end
    ...
    context "with text after code block" do
      ...
      it "extracts code from block with trailing code-like text" do
        ...
        expect(result.code).to include("calculate")
        expect(result.code).to include("final_answer")
      end
    end

    context "with malformed code blocks" do
      it "extracts final_answer even with missing closing backticks" do
        text = "```ruby\nfinal_answer(answer: 42)\n"
        result = described_class.extract_code(text)

        # Standalone final_answer extraction catches this
        expect(result).to be_a(Smolagents::Types::ExtractionResult)
        expect(result).to be_success
        expect(result.code).to eq("final_answer(answer: 42)")
      end

      it "reports truncated when code block is incomplete" do
        text = "```ruby\nresult = search(query:"
        result = described_class.extract_code(text)
      ...
      it "handles extra whitespace everywhere" do
        ...
        expect(result.code).to include("final_answer")
        expect(result.code).to include("50")
      end
    end
    ...
    context "with bare code (no fences or prefix)" do
      it "rejects bare tool calls to avoid executing prose" do
        text = 'result = search(query: "test")'
        result = described_class.extract_code(text)
      ...
      it "extracts code with Action: prefix" do
        text = 'Action: result = search(query: "test")'
        result = described_class.extract_code(text)
      ...
      it "extracts code with Code: prefix" do
        text = "Code: final_answer(answer: 42)"
        result = described_class.extract_code(text)
    ...
    context "with model-specific malformed outputs" do
      ...
      it "combines multiple separate code blocks into one" do
        ...
        expect(result.code).to include("result1 = calculate")
        expect(result.code).to include("result2 = calculate")
        expect(result.code).to include("final_answer(answer: result2)")
      end
    end
  end
  ...
  describe "integration examples" do
    it "combines extraction and pattern matching" do
      ...
      extraction = described_class.extract_code(response_text)

      result = case extraction
               in { code: /final_answer\(.*?(\d+)/ => code }
                 { type: :final, value: code.match(/final_answer\(.*?(\d+)/)[1].to_i }
               in { code: /search\(/ }
                 { type: :search }
               else
                 { type: :unknown }
