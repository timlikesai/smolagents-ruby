...
RSpec.describe Smolagents::Http do
  ...
  describe Smolagents::Http::SsrfProtection do
    ...
    describe ".resolve_and_validate_ip" do
      it "resolves hostname and returns IP" do
      ...
      it "returns nil on DNS resolution failure" do
        allow(Resolv).to receive(:getaddress).and_raise(Resolv::ResolvError)

        result = described_class.resolve_and_validate_ip(URI.parse("https://nonexistent.invalid"))

        expect(result).to be_nil
      end

      it "returns nil on invalid IP address" do
        allow(Resolv).to receive(:getaddress).with("example.com").and_return("not-an-ip")
  ...
  describe Smolagents::Http::Requests do
    ...
    describe "#validate_url!" do
      ...
      it "raises for private IP resolution" do
        allow(Resolv).to receive(:getaddress).with("internal.local").and_return("10.0.0.1")
