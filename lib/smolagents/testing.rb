  ...
  module Testing
    module Helpers
      def mock_model_that_responds(response, tool_calls: nil)
        message = response.is_a?(ChatMessage) ? response : (tool_calls ? ChatMessage.assistant(response, tool_calls: tool_calls.map { |tc| ToolCall.new(**tc) }) : ChatMessage.assistant(response))
      ...
      def mock_tool(name, returns: nil, raises: nil)
        Class.new(Tool) { self.tool_name = name; self.description = "Mock #{name} tool"; self.inputs = { "input" => { "type" => "string", "description" => "Input" } }; self.output_type = "string"; define_method(:forward) { |**_| raise raises if raises; returns } }.new
      end

      def spy_tool(name) = SpyTool.new(name)
      def test_agent(model_response:, tools: [], agent_class: nil) = (agent_class || (defined?(CodeAgent) ? CodeAgent : MultiStepAgent)).new(model: mock_model_that_responds(model_response), tools: tools)
      def capture_agent_steps(agent) = [].tap { |steps| agent.register_callback(:on_step_complete) { |_, monitor| steps << monitor }; yield }
      def assert_agent_success(result) = (expect(result).not_to be_nil; expect(result).to be_a(String).or be_a(Hash).or be_a(ActionOutput))
    ...
    module Fixtures
      ...
      def self.action_step(step_number: 1, **kwargs) = ActionStep.new(step_number: step_number).tap { |s| s.timing = Timing.start_now; kwargs.each { |k, v| s.send("#{k}=", v) } }
    ...
    module Matchers
      RSpec::Matchers.define :complete_successfully do
        match { |agent_or_result| @result = agent_or_result.respond_to?(:run) ? agent_or_result.run(@task) : agent_or_result; !@result.nil? && !@result.is_a?(Exception) }
      ...
      RSpec::Matchers.define :call_tool do |tool_name|
        match { |actual| @actual_calls = actual.is_a?(SpyTool) ? actual.calls : actual; @actual_calls.any? { |call| call.is_a?(Hash) && (!@expected_args || @expected_args.all? { |k, v| call[k] == v || call[k.to_s] == v }) } }
