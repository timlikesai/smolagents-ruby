...
module Smolagents
  module Concerns
    ...
    module Retryable
      ...
      def with_retry(max_attempts: DEFAULT_CONFIG[:max_attempts], base_delay: DEFAULT_CONFIG[:base_delay], max_delay: DEFAULT_CONFIG[:max_delay], exponential_base: DEFAULT_CONFIG[:exponential_base], jitter: DEFAULT_CONFIG[:jitter], on: [StandardError])
        attempts = 0
        loop do
          attempts += 1
          return yield
