  ...
  module Tools
    class Tool
      ...
      module ErrorHints
        ...
        HINT_HANDLERS = {
          NameError => ->(e, inputs, _tn, _ei) { ErrorHints.hint_for_name_error(e, inputs) },
          TypeError => ->(e, _i, _tn, _ei) { ErrorHints.hint_for_type_error(e) },
          ArgumentError => ->(e, _i, tn, ei) { ErrorHints.hint_for_argument_error(e, tn, ei) },
          NoMethodError => ->(e, _i, _tn, _ei) { ErrorHints.hint_for_no_method_error(e) },
          ZeroDivisionError => ->(*_) { "Check your expression for division by zero." }
        }.freeze

        def self.find_best_hint(error, tool_name:, inputs:, expected_inputs: nil)
          handler = HINT_HANDLERS.find { |klass, _| error.is_a?(klass) }&.last
          handler&.call(error, inputs, tool_name, expected_inputs)
        end
        ...
        def self.hint_for_argument_error(error, tool_name, expected_inputs)
          msg = error.message
          valid_kw = format_valid_keywords(expected_inputs)
          return wrong_number_hint(tool_name, valid_kw) if msg.include?("wrong number of arguments")
          return missing_keyword_hint(msg, tool_name) if msg.include?("missing keyword")

          unknown_keyword_hint(msg, tool_name, valid_kw) if msg.include?("unknown keyword")
        end

        def self.wrong_number_hint(tool_name, valid_kw)
          "#{tool_name} is a function. Call it: #{tool_name}(#{valid_kw || "arg: value"})"
        end

        def self.missing_keyword_hint(msg, tool_name)
          return unless (match = msg.match(/missing keyword:\s*:?(\w+)/))

          "Missing required argument. Use: #{tool_name}(#{match[1]}: \"...\")"
        end

        def self.unknown_keyword_hint(msg, tool_name, valid_kw)
          wrong = msg.match(/unknown keyword:\s*:?(\w+)/)&.[](1)
          prefix = wrong ? "Unknown '#{wrong}:'." : "Wrong argument name."
          return "#{prefix} Valid arguments: #{valid_kw}" if valid_kw

          "#{prefix} Use help(:#{tool_name}) for correct arguments."
        end
